...
class TestNotebookControllerDeployment:
    ...
    def test_controller_replicas_ready(
        ...
        assert len(pods) == EXPECTED_REPLICAS, (
            f"Expected {EXPECTED_REPLICAS} pod(s) for {controller_name}, found {len(pods)}"
        )
    ...
    def test_controller_container_count(
        ...
        assert pods, f"No pods found for {controller_name} in namespace {namespace}"
    ...
    def test_controller_container_resources(
        ...
        for pod in pods:
            ...
            actual_limits = {
                "cpu": manager_container.resources.limits.get("cpu"),
                "memory": manager_container.resources.limits.get("memory"),
            }
            actual_requests = {
                "cpu": manager_container.resources.requests.get("cpu"),
                "memory": manager_container.resources.requests.get("memory"),
            }
